...
const copiaClase_1 = require("../clases/copiaClase");
const copiaMetodo_1 = require("../clases/copiaMetodo");
const Parametro_1 = require("../clases/Parametro");
const copiaVariable_1 = require("../clases/copiaVariable");
class IndexRoutes {
    constructor() {
        this.fs = require("fs");
        this.errlex = "";
                                                                ...
                                                                if (this.analisis2.instructions[j].instrucciones[l].instrucciones[n].tipoInstruccion == "declaracion") {
                                                                    if (this.analisis.instructions[i].instrucciones[k].instrucciones[m].identificador == this.analisis2.instructions[j].instrucciones[l].instrucciones[n].identificador) {
                                                                        if (this.analisis.instructions[i].instrucciones[k].instrucciones[m].tipo.type == this.analisis2.instructions[j].instrucciones[l].instrucciones[n].tipo.type) {
                                                                            console.log("iguales");
                                                                            this.listaVariable.push(new copiaVariable_1.copiaVariable(this.analisis.instructions[i].instrucciones[k].instrucciones[m].tipo.toString0(), this.analisis.instructions[i].instrucciones[k].instrucciones[m].identificador, this.analisis.instructions[i].instrucciones[k].identificador, this.analisis.instructions[i].identificador));
                                                                        }
                                                                    }
                                                                }
                                                            }
                                                ...
                                                if (this.analisis.instructions[i].instrucciones[k].parametros.length == this.analisis2.instructions[j].instrucciones[l].parametros.length) {
                                                    ...
                                                    for (var m = 0; m < this.analisis.instructions[i].instrucciones[k].parametros.length; m++) {
                                                        if (this.analisis.instructions[i].instrucciones[k].parametros[m].tipo.type == this.analisis2.instructions[j].instrucciones[l].parametros[m].tipo.type) {
                                                            iguales = true;
                                                            parametros.push(new Parametro_1.Parametro(this.analisis.instructions[i].instrucciones[k].parametros[m].tipo.toString0(), this.analisis.instructions[i].instrucciones[k].parametros[m].identificador));
                                                        }
                                                        else {
                                                            iguales = false;
                                                            break;
                                                    ...
                                                    if (!iguales) {
                                                        ...
                                                        console.log("iguales");
                                                        this.listaMetodo.push(new copiaMetodo_1.copiaMetodo(this.analisis.instructions[i].instrucciones[k].tipo.toString0(), this.analisis2.instructions[j].instrucciones[l].identificador, this.analisis.instructions[i].identificador, parametros));
                                                    }
                                                }
                                            }
                                        }
                        ...
                        if (this.analisis2.instructions[j].tipoInstruccion == "clase") {
                            if (this.analisis.instructions[i].identificador == this.analisis2.instructions[j].identificador) {
                                ...
                                if (numMetodosIguales == contadorA2) {
                                    ...
                                    this.listaClase.push(new copiaClase_1.copiaClase(this.analisis.instructions[i].identificador, contadorA1));
                                    return;
                                }
                            }
                            else {
